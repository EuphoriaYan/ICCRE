...
class GibbsSampler:
    def __init__(self):
        self.data_loader = CWSDataLoader()

    def sample(self, freq_dict, text, max_iters):
        self.data_loader.update_freq_dict(lines=text, freq_dict=freq_dict)
        ...
        return [' '.join(sent[1:len(sent)-1]) for sent, _ in all_sent_tag]
